...
input_rows		=		4
input_cols		=		4
input_total		=		input_rows*input_cols
...
filter_weights = (np.random.rand(filter_depth, filter_rows, filter_cols)*(1/input_total))
...
def cnn_forward(input_layer, input_filter):
	...
	# conv_layer_sigmoid = relu_function(conv_layer)
	...
	# hidden_layer_sigmoid = relu_function(hidden_layer)
	...
	# Return Predicted
	return output_layer, output_layer_softmax

def cnn_back(input_layer, input_filter, fc1, fc2, target_value):
	##### START OF FORWARD PASS
	# Convolution
	for i in range(0, input_filter.shape[0]):
		conv_layer[i] = signal.convolve(input_layer, input_filter[i], mode="valid")
	...
	# slope_output_layer = relu_function(output_layer_softmax)
	slope_hidden_layer = sigmoid_function(hidden_layer_sigmoid, deriv=True)
	# slope_hidden_layer = relu_function(hidden_layer_sigmoid)
	slope_conv_layer = sigmoid_function(conv_layer_sigmoid_flat, deriv=True)
	# slope_conv_layer = relu_function(conv_layer_sigmoid_flat)
...
for z in range(0,epoch):
	...
	f,fc1,fc2,out,out_s = cnn_back(input_layer, f, fc1, fc2, 1)

	input_layer = get_image(image2)
	f,fc1,fc2,out,out_s = cnn_back(input_layer, f, fc1, fc2, 2)

	# input_layer = get_image(image3)
	# f,fc1,fc2,out,out_s = cnn_back(input_layer, f, fc1, fc2, 3)

	# input_layer = get_image(image4)
	# f,fc1,fc2,out,out_s = cnn_back(input_layer, f, fc1, fc2, 4)
...
print("FC1 after iterations:")
print(fc1)

print("FC2 after iterations:")
print(fc2)
...
input_layer = get_image(image1)
out,out_s = cnn_forward(input_layer, f)

print("Output Layer Of Test Image:")
print(out)
print(out_s)
