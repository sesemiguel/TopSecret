...
mnist_train_random_target = np.zeros(30)

mnist_train = pd_csv_to_2darray("mnist_test.csv")
# print(mnist_train)
...
for i in range(0, mnist_d):
	r = randint(0,10000)
	mnist_train_batch[i] = np.reshape(mnist_train_clean[r,:], (mnist_r, mnist_c)) #EXTRACT DATA AND RESHAPE TO SPECIFIC ROW COL
	print("Matrix Number: " + str(i))
	mnist_train_random_target[i] = mnist_train_target[r]
	print(r)
...
maxpooled_array = np.where(maxpooled_array > 65, 1, 0)

pd_3darray_to_csv(maxpooled_array, "mnist_30_digit_3d_9x9_test_subset_4.csv")
...
np.savetxt("mnist_30_digit_maxpooled_9x9_test_subset_4.csv", maxpooled_array, delimiter=",")

np.savetxt("30_digit_target_subset_4.csv", mnist_train_random_target, delimiter=",")